...
void print_board(const vector<vector<int>>& board) {
	...
	for(const auto& row : board)    // Looping through each rows in the grid vector
	{
		for(const auto& cell: row) // Looping through the cell value for each row
		{
			if(cell == 0)
				cout << ".\t";
			else
...
bool move_left(std::vector<std::vector<int>>& board) {
	...
	for(auto& row: board)
	{
		const std::vector<int> new_row = merge_row(row);
		if(new_row != row)
		{
			moved = true;
			row = new_row;
...
bool move_up(std::vector<std::vector<int>>& board) {
	...
	for(int c = 0; c < 4; c++)
	{
		...
		std::vector<int> new_col = merge_row(col);
...
bool move_down(std::vector<std::vector<int>>& board) {
	...
	for(int c = 0; c < 4; c++)
	{
		...
		std::vector<int> original_col = col;		
